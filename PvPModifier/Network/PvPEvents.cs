    ...
    public class PvPEvents {
            ...
            if (target != null) {
                ...
                foreach (var pvper in PvPModifier.ActivePlayers) {
                    pvper.SendRawData(new PacketWriter()
                        .SetType((byte)PacketTypes.ProjectileNew)
        ...
        private void OnPlayerHurt(object sender, PlayerHurtArgs e) {
            ...
            if (PvPModifier.Config.EnableBuffs) {
                e.Target.SetBuff(Cache.Projectiles[(int)e.PlayerHitReason.SourceProjectileType].InflictBuff);
                e.Attacker.SetBuff(Cache.Projectiles[(int)e.PlayerHitReason.SourceProjectileType].ReceiveBuff);
